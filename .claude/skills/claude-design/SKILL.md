...
## Monorepo Component Library Pattern

When the user asks to "design" a page in an existing monorepo (Turborepo + pnpm workspaces, Nx, etc.), **do not write raw CSS/Tailwind inline**. Instead, create or extend a shared component package:

1. **Create `packages/ui/`** (or extend it) with:
   - `tokens.ts` — design tokens (colors, typography, spacing, radii, platform-specific palettes)
   - `utils.ts` — `cn()` utility (`clsx` + `tailwind-merge`)
   - Base components using `class-variance-authority` (cva) for variant-driven APIs
   - Domain components specific to the product (ProductCard, FilterBar, ProfileHeader)
   - `index.ts` barrel export

2. **Layer the components**:
   - **Primitives**: Button, Badge, Avatar, Card, Input (variant-driven, cva)
   - **Layout**: Container, Stack, Separator (composition helpers)
   - **Domain**: ProductCard, FilterBar, Nav, Footer (product-specific, may compose primitives)

3. **Update `apps/web/`** pages to import from `@affbio/ui` (or `@<org>/ui`), not inline styles.

4. **Dependencies to add**: `clsx`, `tailwind-merge`, `class-variance-authority` (in the ui package).

5. **Verify**: `pnpm --filter @<app>/web build` must pass. Then restart the service.

See `references/monorepo-component-library.md` for the full worked example (affbio.link).

## Pitfalls

- **`'use client'` for interactive components in shared packages.** In Next.js App Router, components with event handlers (`onClick`, `onChange`, etc.) in a shared `packages/ui/` MUST have `'use client'` at the top of the file. Server components in `apps/web/` cannot pass event handlers to client component props without this directive — runtime error: "Event handlers cannot be passed to Client Component props."
- **systemd + Next.js: use `node` directly.** The `node_modules/.bin/next` is a shell script wrapper, not a Node.js file. In systemd `ExecStart=`, use: `ExecStart=/path/to/node /path/to/node_modules/next/dist/bin/next start -p <port>`. Using the `.bin/next` wrapper directly causes `SyntaxError: missing ) after argument list`.
- **PostgreSQL permissions: `GRANT ALL ON SCHEMA public` is not enough.** After creating a database and user, also run: `GRANT ALL ON ALL TABLES IN SCHEMA public TO user; ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO user;`. Tables created by the `postgres` superuser are not accessible by the app user otherwise. Best practice: have the app user create its own tables (connect as that user for migrations).
