 ...
 *
 * GOAL #4 requires that EVERY "leak" verdict ship a root-cause explanation AND
 * an applicable fix diff. The LLM judge already does this opportunistically, but
 * the heuristic judge (the only path in no_llm mode, and the fallback whenever
 * no LLM key is configured) produced neither. This module fills that gap with a
 * deterministic, source-anchored analysis:
 *
 ...
 */
import { LeakBundle, LeakCandidate, LeakRootCause, LeakPatternType, RepairDiff } from '@mcpvul/common';
