...
describe('table renderers', () => {
  const rows: BaselineSweepRow[] = [
    ...
    { id: 'B7', name: 'Full adaptive', status: 'ok', ranOk: 198, caseCount: 200, runs: 3, tp: 20, fp: 2, fn: 0, tn: 66, precision: 0.79, recall: 0.95, f1: 0.86, f1Std: 0.01, fpPerKloc: 1.0, ece: 0.12, meanDurationMs: 8000, meanMcpCalls: 14, meanTokens: 5200, totalTokens: 1040000, totalCostUsd: 12.5 },
  ...
  test('markdown has a token-cost footer with the sweep grand total, incl. $ cost', () => {
    const md = renderSweepMarkdown(rows, { corpus: 'demo/juliet_cwe401', limit: 200 });
    ...
    expect(md).toContain('$12.50'); // B7's totalCostUsd
    expect(md).toContain('$ cost');
  });

  test('an unpriced row renders "—" for $ cost, not $0', () => {
    const unpriced: BaselineSweepRow[] = [{ ...rows[1], totalCostUsd: undefined }];
    const md = renderSweepMarkdown(unpriced, { corpus: 'demo/juliet_cwe401' });
    expect(md).toContain('| B7 | Full adaptive | 1040000 | 5200 | — |');
  });

  test('csv has a header + one line per row with confusion + ece + cost', () => {
    ...
    expect(lines[2]).toContain(',12.5'); // B7's totalCostUsd, trailing column
  });

  test('latex emits only ok rows with confusion + ECE + $ cost', () => {
    ...
    expect(tex).toContain('B7 & Full adaptive');
    expect(tex).not.toContain('Dynamic only'); // skipped row excluded
    expect(tex).toContain('\\begin{tabular}');
    expect(tex).toContain('$12.50'); // B7's totalCostUsd
    expect(tex).toContain('B1 & Static only & 19 & 3 & 1 & 65 & 80.6% & 90.6% & 0.853 & 0.741 & 0.593 & 3 & 0 & — \\\\'); // B1 unpriced → em dash
  });
});
