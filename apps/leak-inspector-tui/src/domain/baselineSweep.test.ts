...
describe('table renderers', () => {
  const rows: BaselineSweepRow[] = [
    { id: 'B1', name: 'Static only', status: 'ok', ranOk: 200, caseCount: 200, runs: 1, precision: 0.806, recall: 0.906, f1: 0.853, fpPerKloc: 0.741, meanDurationMs: 1200, meanMcpCalls: 3, meanTokens: 0 },
    { id: 'B7', name: 'Proposed', status: 'ok', ranOk: 198, caseCount: 200, runs: 3, precision: 0.79, recall: 0.95, f1: 0.86, f1Std: 0.01, fpPerKloc: 1.0, meanDurationMs: 8000, meanMcpCalls: 14, meanTokens: 5200 },
    { id: 'B2', name: 'Dynamic only', status: 'skipped', skipReason: 'dynamic-only discovery not yet implemented (Step 4a)' },
  ];

  test('markdown has a row per baseline and shows mean±std for multi-run F1', () => {
    const md = renderSweepMarkdown(rows, { corpus: 'demo/juliet_cwe401', limit: 200 });
    expect(md).toContain('| B1 | Static only | 200/200 |');
  ...
  test('csv has a header + one line per row', () => {
    const csv = renderSweepCsv(rows);
    const lines = csv.trim().split('\n');
    expect(lines[0]).toStartWith('id,name,status');
    expect(lines).toHaveLength(1 + rows.length);
    expect(lines[1]).toContain('B1,"Static only",ok');
  });

  test('latex emits only ok rows', () => {
    const tex = renderSweepLatex(rows, { corpus: 'demo/juliet_cwe401' });
    expect(tex).toContain('B1 & Static only');
