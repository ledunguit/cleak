...
function printProgressLine(run: Extract<BaselineInspection['runs'][number], { final: false }>): void {
  const p = run.progress;
  const llm = p.judgePathCounts.llm ?? 0;
  const heuristic = p.judgePathCounts.heuristic ?? 0;
  const llmPct = llm + heuristic > 0 ? pct(llm / (llm + heuristic)) : '—';
  ...
  console.log(
    `   run-${run.run}: ▶ ${p.done} case(s) so far` +
      ...
      ` · judge llm% ${llmPct} · FP/KLOC ${p.fpPerKloc.toFixed(3)}` +
      (liveF1 !== undefined ? ` · live F1≈${liveF1.toFixed(3)}` : ''),
  );
}
    ...
    for (const i of partial) {
      ...
      for (const r of i.runs) {
        if (r.final) {
          ...
          console.log(`   run-${r.run}: ✓ ${r.result.ranOk}/${r.result.caseCount} · P ${pct(m.precision)} R ${pct(m.recall)} F1 ${m.f1.toFixed(3)}`);
        } else {
          printProgressLine(r);
        }
      }
