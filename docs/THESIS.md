...
| **LAMeD (7 dự án thực, 50 site — đã sửa mẫu số từ 44)** | `no_llm` và `llm_assisted` (3 lần chạy, std=0) cho **kết quả y hệt nhau**: P100%/R30.0% (TP15/FP0/FN35). `interproceduralFlow` không còn tạo khác biệt — đã trace: path-insensitivity trong chính cơ chế đếm alloc/free của tool, không phải bug wiring. Sửa lại claim cũ (+1 TP `merge_patch` trên mẫu 44-site tiền-fix) — xem [EVALUATION.md](EVALUATION.md) |
