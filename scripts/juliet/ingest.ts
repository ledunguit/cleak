...
const cases = testcases.map((tc) => {
  ...
  if (buildable) {
    ...
    const testcaseBasenames = tc.files.map((f) => basename(f));
    writeFileSync(join(caseDir, 'Makefile'), generateJulietMakefile(caseDir, testcaseBasenames));
  }
  ...
  return {
    ...
    ...(buildable ? { build_command: JULIET_BUILD_COMMAND } : {}),
    flaws,
    clean,
