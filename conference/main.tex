...
seven real-world C projects from the peer-reviewed LAMeD
benchmark, the deterministic (\texttt{no\_llm}) configuration achieves 100\%
precision with recall rising from 0.250 to 0.273 as cross-function ownership
evidence (\texttt{interproceduralFlow}) is added, including one
previously-unreported cJSON leak caught end-to-end; a separate, not-yet
re-measured \texttt{llm\_assisted} run on a broader case set showed LOWER
recall than this deterministic baseline, a gap we report rather than hide
...
estimate for the pending re-measurement of \texttt{llm\_assisted} LAMeD
recall (Section~\ref{sec:eval-lamed}); it is reported as open rather than
assumed favorable. The dynamic-augmented full-corpus run flagged as
pending in an earlier version of this evaluation is no longer open ---
...
mechanism rather than leaving unexplained); and on the real-world, peer-reviewed
...
Future work has two concrete, already-identified directions (a third,
running the dynamic stage on the full 1{,}658-case Juliet corpus, is no
longer open --- Section~\ref{sec:eval-fullcorpus} now reports it, as noted
above): evaluating the opt-in targeted-harness
