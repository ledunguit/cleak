...
LLM-driven tool selection is $\sim$4$\times$ more expensive in dollar cost
...
sites), a heterogeneity a family-balanced sub-sample would average away. On
seven real-world C projects from the peer-reviewed LAMeD benchmark (50
scoreable sites after correcting a site-undercounting bug traced to the same
candidate-deduplication fix discussed below; see
Section~\ref{sec:eval-lamed}), the deterministic (\texttt{no\_llm})
configuration achieves 100\% precision at 30.0\% recall; a separately
re-measured \texttt{llm\_assisted} configuration (mean of 3 independent runs)
reaches the identical confusion matrix, and adding
\texttt{interproceduralFlow} makes no measurable difference on this corpus
today. We traced the latter directly to a path-sensitivity gap in
\texttt{interproceduralFlow}'s own alloc/free counting, not a wiring defect,
and report it as an honest correction to an earlier draft's claim rather than
omit it (Section~\ref{sec:eval-lamed}). We report all of
...
\item[C4: A self-consistency consensus judge, honestly evaluated.] A
  judging layer that fuses static and dynamic evidence and, only for
  genuinely borderline bundles, resamples the LLM judge $K$ times at
  nonzero temperature, combining the votes by a self-consistency rule. We
  report this as an \emph{investigated technique with a
  sample-composition-dependent finding} rather than a proven win: it
  reduces verdict flip rate on a small, accidentally family-homogeneous
  sample, but that advantage reverses on a properly family-stratified
  sample of comparable size, a methodological lesson we report in full
  rather than keep only the favorable number (Section~\ref{sec:eval-consensus}).
\end{description}
...
approach; and where CLeak does rely on across-run reuse to avoid redundant
...
benchmark --- a result CLeak's own architecture does not contest, since
...
uniform (Table~\ref{tab:family}); we report the full distribution rather
...
\emph{lower} F1. B4--B7 are the mean of 3 runs; std $\le 0.005$ on F1
...
cases; one case can score multiple sites). \texttt{destructor}/\texttt{virtual}
...
\subsection{Consensus judge: stability and significance --- an honest, reversed finding}
\label{sec:eval-consensus}

Table~\ref{tab:consensus} compares single-LLM ($K{=}1$) against consensus
($K{=}3$) judging across two independent campaigns of an $n{=}30$
...
consensus, but $\chi^2 = 0.57$, $p = 0.45$: not significant at
this sample size.

\textbf{This $n{=}30$ sample is not family-stratified.} Juliet's manifest
orders cases by functional family, so an unstratified first-$N$ slice at
this size draws almost entirely from a single family; we confirmed this
sample is 100\% \texttt{char}-family cases, one of the corpus's
easier, more homogeneous families (Table~\ref{tab:family}). To test
whether the stability advantage above survives on a sample that actually
represents the corpus's difficulty spread, we repeated the identical
protocol --- same script, same judge, same rule, two independent
campaigns --- on a family-\emph{stratified} $n{=}50$ sample (6 cases each
from 8 families, 1 each from the 2 smallest), archived at
\texttt{results/consensus-ablation-n50-2026-08-19/}.

\begin{table}[htbp]
\centering
\caption{Same protocol, family-stratified $n{=}50$: the stability advantage
\emph{reverses}.}
\label{tab:consensus-n50}
\small
\begin{tabular}{@{}llccc@{}}
\toprule
\textbf{Judge arm} & \textbf{Campaign} & \textbf{Case stability} & \textbf{Flip rate} & \textbf{Modal agreement} \\
...
On the stratified sample, single-LLM is now the \emph{more} stable arm
(flip rate 2.0\% vs.\ consensus's 8.0\%) and the more accurate one (F1
0.852--0.855 across its two campaigns vs.\ consensus's 0.793--0.796,
consistently in both). The paired McNemar test (campaign A, 205 sites)
leans the same direction --- single-LLM accuracy 91.7\%/F1~0.852 vs.\
consensus 89.0\%/F1~0.793, 7 of 8 discordant sites favoring single-LLM,
$\chi^2 = 3.13$, $p = 0.077$ --- again not significant at this sample
size, but now pointing the opposite way from the $n{=}30$ result above.

\textbf{We report both results in full rather than keep only the
favorable one.} The most defensible reading is that the $n{=}30$ result
is a sampling artifact: on a corpus slice drawn from one easy, homogeneous
family, three independent judge samples usually agree trivially (the
correct answer is unambiguous), so voting looks like it is "stabilizing"
what was never unstable to begin with; averaged across a difficulty
spread that includes genuinely borderline families (e.g.\
\texttt{strdup}, Table~\ref{tab:family}), three independent samples of an
\emph{uncertain} judgment can converge on a shared systematic error just
as easily as they converge on the correct answer, which self-consistency
voting cannot distinguish from converging on truth. Neither $n{=}30$ nor
$n{=}50$ has the statistical power to settle this on its own (McNemar is
not significant either way); what the reversal \emph{does} establish is
that consensus judging's benefit, if real, is not sample-composition-independent, and that C4's contribution is best read as this
methodological finding --- family-stratified sampling matters for
evaluating an LLM-judge technique, not just for evaluating the end system
--- rather than as a validated accuracy or stability improvement. We do
...
pending a larger, stratified, multi-seed study.

\subsection{Real-world validation: LAMeD}
\label{sec:eval-lamed}

On the 41-case, 7-project LAMeD benchmark, the deterministic \texttt{no\_llm}
configuration holds \textbf{100\% precision (zero false positives) in every
configuration we measured}, with recall at \textbf{30.0\%} (Table~\ref{tab:lamed}).
A manual taxonomy of the six confirmed cJSON leaks in this benchmark finds
...
\textbf{Site count correction.} An earlier version of this evaluation
reported 44 scoreable sites; the corrected count is \textbf{50}. Both
figures come from the same 41 cases and the same LAMeD manifest --- nothing
external changed. The difference is a fix to \texttt{computeBundleId}, which
used a truncating hash that collapsed genuinely distinct candidates into one
bundle (verified on one project alone to shrink 622 raw candidates to 65
surviving bundles pre-fix). Because our scorer counts scoreable sites as
bundles that match a ground-truth leak, not raw manifest entries, the same
collapsing bug that suppressed recall also \emph{under-counted the
denominator} on cases with multiple call sites for the same flaw: libsolv
goes from 6 to 11 sites and cJSON from 6 to 7; the other five projects
(curl, libssh2, libtiff, libxml2, rabbitmq-c) are unaffected. Every number in
Table~\ref{tab:lamed} is measured post-fix, on the 50-site denominator, with
the current container images rebuilt from source. \textbf{The Clang Static
Analyzer row's 43-site denominator is not an artifact we can fix by
re-running it.} Our scorer (\texttt{scoreCase}) derives the scoreable-site
set from whichever tool's own findings are being graded: it groups that
tool's findings into site-keys, matches them against manifest flaws, and
adds one synthetic false negative per flaw no finding covers. The site
count is therefore a property of \emph{each tool's own finding
granularity}, not a single external ground truth shared across tools ---
CLeak's count moved from 44 to 50 because \texttt{computeBundleId} changed
how CLeak's own candidates bundle, which has no bearing on Clang Static
Analyzer's independently-produced findings. We re-ran Clang Static Analyzer
under the current harness to confirm this directly: it still scores 43
sites, TP\,0/FN\,43, unchanged. The CSA and CLeak rows in Table~\ref{tab:lamed}
are therefore not on a shared site denominator by design, not by omission;
we report both counts as measured rather than force them into a single
column that would misstate one of them.

\textbf{\texttt{interproceduralFlow} adds no measurable value on this
benchmark today.} Unlike an earlier version of this evaluation, which
credited \texttt{interproceduralFlow} with catching cJSON's
\texttt{merge\_patch} leak, the default and \texttt{+interproceduralFlow}
configurations are now byte-identical across all 41 cases
(Table~\ref{tab:lamed}), including on \texttt{merge\_patch} itself. We
traced this directly by calling \texttt{interproceduralFlow} on
\texttt{merge\_patch} in isolation: it reports \texttt{allocCount=1,
freeCount=2, "balanced"} and \texttt{hasAllocWithoutFree=false}, because it
counts two \texttt{cJSON\_Delete(target)} call sites reachable from
\texttt{merge\_patch} and concludes the allocation is freed. It does not
check whether those two frees are mutually exclusive on different branches
--- they are: the real bug is a \emph{third} branch, an error path, that
frees neither. This is not a wiring defect (the tool runs and returns a
well-formed trace) and not a regression from an unrelated code change; it is
the same fundamental limitation as the judge's own guard-subset path
reconciliation (Section~\ref{sec:discussion}) --- alloc/free counting that is
not path-sensitive --- except here it lives inside
\texttt{interproceduralFlow}'s own cross-function trace rather than the
judge layer. We correct the earlier claim rather than let it stand
uncontested: on this benchmark, as currently implemented,
\texttt{interproceduralFlow} contributes zero measured recall beyond the
default static recipe.
...
variance across runs (Section~\ref{sec:eval-lamed}). CLeak rows score 50
scoreable sites (positive-only, TN$=$0 by construction; corrected from an
earlier reported 44 --- see site-count correction above); the Clang Static
Analyzer row scores 43 sites under a live re-run with the same scorer ---
the two counts are tool-dependent by the scorer's own design (see
discussion above) and are not on a shared denominator.}
\label{tab:lamed}
\small
...
\textbf{\texttt{llm\_assisted} re-measurement.} An earlier version of this
evaluation flagged an \texttt{llm\_assisted} recall of 15.9\% on this
benchmark as stale, attributing it to the same pre-fix candidate-deduplication
bug and leaving the corrected number as future work. We have now completed
that re-measurement (Table~\ref{tab:lamed}): \texttt{llm\_assisted} reaches
the identical confusion matrix as \texttt{no\_llm} (TP\,15/FP\,0/FN\,35), with
...
heuristic's own decision on this corpus. The fix removed the earlier
regression --- \texttt{llm\_assisted} no longer under-performs \texttt{no\_llm}
--- but it does not exceed it either, on this real-project benchmark. This
is consistent with Section~\ref{sec:eval-ablation}'s Juliet finding that
...
elsewhere in this paper (Table~\ref{tab:lamed}). This is the SAME
unstratified, family-homogeneous $n{=}30$ sample discussed in
Section~\ref{sec:eval-consensus} --- the Consensus row's favorable numbers
here should be read together with that section's honest, reversed finding
on a stratified sample, not in isolation.}
\label{tab:external}
\small
...
technique, not just for evaluating the end system.} Section~\ref{sec:eval-consensus}'s
reversal --- consensus voting stabilizes verdicts on an unstratified,
family-homogeneous $n{=}30$ sample but not on a family-stratified $n{=}50$
sample of comparable size --- is, to our knowledge, not a result specific
to our consensus mechanism; it is a caution about evaluating any
self-consistency technique on a sample small enough that a single
functional family can dominate it by manifest ordering alone. We flag
this as a general methodological point for LLM-judge evaluation
protocols, not only a limitation of this particular ablation.

\textbf{Measurements closed since an earlier version of this evaluation.}
Two items previously flagged as open are now reported rather than assumed.
The \texttt{llm\_assisted} LAMeD re-measurement (Section~\ref{sec:eval-lamed})
is complete: it matches \texttt{no\_llm} exactly, not the favorable
projection we might have hoped the dedup fix would produce, and we report
that flat result rather than the more flattering unmeasured guess. The
dynamic-augmented full-corpus run is likewise no longer open ---
Section~\ref{sec:eval-fullcorpus} now reports it, with dynamic evidence
enabled across all nine capability configurations.
...
and parameter-ownership leaks without an SMT solver; and a self-consistency
consensus judge, evaluated honestly rather than assumed to help, stabilizes
borderline verdicts on a small, family-homogeneous sample but does not
replicate that advantage on a family-stratified sample of comparable
size --- a methodological finding about LLM-judge evaluation we report
in full rather than the cleaner, favorable half of it. A controlled five-factor
ablation on the full validated Juliet corpus (1{,}658 cases, not a
sub-sample) isolates a deterministic static+dynamic+planner configuration
...
mechanism rather than leaving unexplained); and on the real-world,
peer-reviewed LAMeD benchmark the system holds 100\% precision at 30.0\%
recall, identically under \texttt{no\_llm} and a separately re-measured
\texttt{llm\_assisted} configuration (Section~\ref{sec:eval-lamed}) --- a
result we report plainly rather than substitute with an earlier, since-corrected
number that credited \texttt{interproceduralFlow} with a catch it does not,
on inspection, actually make.

Future work has one concrete, already-identified direction: evaluating the
opt-in targeted-harness escalation stage (per-candidate harness synthesis
plus a bounded fuzzing tier for bundles a whole-binary run leaves borderline)
mentioned but not measured in this paper. Two items closed during this
evaluation cycle rather than remaining open: the dynamic stage now runs on
the full 1{,}658-case Juliet corpus (Section~\ref{sec:eval-fullcorpus}), and
\texttt{llm\_assisted} recall on LAMeD has been re-measured under the
candidate-deduplication fix (Section~\ref{sec:eval-lamed}) --- the fix
removed a prior regression but did not, as we had speculated it might, raise
real-project recall above the deterministic baseline; \texttt{interproceduralFlow}'s
path-insensitivity on this same benchmark (Section~\ref{sec:eval-lamed}) is
a natural next target, since a path-sensitive cross-function trace is a
smaller lift than a full SMT-based solver.
