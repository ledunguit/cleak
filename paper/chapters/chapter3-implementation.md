...
Hệ thống được viết hoàn toàn bằng TypeScript, chạy trên nền Node.js, quản lý qua pnpm. Đây là quyết định có phần đi ngược số đông — phần lớn công cụ phân tích code C/C++ (Clang SA [1], Infer, CodeQL) đều viết bằng C++ hoặc OCaml. Tuy nhiên, mục tiêu của luận văn không phải xây thêm một analyzer từ đầu, mà xây một **orchestrator** điều phối nhiều analyzer có sẵn qua giao thức MCP [42]. Với bài toán này, TypeScript có mấy lợi thế cụ thể:
...
Runtime Bun được thử nghiệm ban đầu vì tốc độ khởi động nhanh hơn Node.js trong giai đoạn phát triển sớm. Tuy nhiên, khi vận hành CI/CD (GitHub Actions, Ubuntu Linux), hệ thống gặp một lỗi tương thích nghiêm trọng: gói `tree-sitter` — native addon biên dịch qua node-gyp — không phát hành sẵn bản build cho `linux-x64`; khi chạy dưới runtime Bun, cơ chế resolve module của gói này rẽ nhánh thẳng tới một đường dẫn `prebuilds/` cố định, bỏ qua fallback `node-gyp-build` vốn biết tìm bản build biên dịch tại chỗ (`build/Release/`). Hệ quả: mọi lệnh parse AST trên CI thất bại âm thầm (lỗi bị nuốt bởi một khối `try/catch` phòng thủ), trả về kết quả rỗng mà không có log lỗi rõ ràng — một lớp bug khó phát hiện trong chính pipeline đánh giá tự động của luận văn. Sau khi xác định nguyên nhân gốc, hệ thống được chuyển sang Node.js làm runtime (dev, test, script) và pnpm làm package manager, đồng thời thay `bun:test` bằng Vitest. Quyết định này ưu tiên tính tương thích và độ ổn định của pipeline CI/CD hơn tốc độ khởi động thô — vốn không phải nút thắt cổ chai thực sự trong một hệ thống mà độ trễ chủ yếu đến từ lời gọi LLM, không phải từ runtime JavaScript.
